...
CPP_FILES := $(wildcard $(SRC_DIRS)/*.cpp $(SRC_DIRS)/**/*.cpp )
# CPP_FILES = $(SRC_DIRS)/main.cpp
HEADER_FILES := $(wildcard $(SRC_DIRS)/**/*.h $(SRC_DIRS)/*.h)
OBJS := $(CPP_FILES:%.cpp=$(BUILD_DIR)/%.o)
DEPS := $(CPP_FILES:%.cpp=$(BUILD_DIR)/%.d)
# OBJS = $(SRCS:.cpp=.o)\
...
$(BUILD_DIR)/%.d: %.cpp
	$(MKDIR_P) $(dir $@)
	$(CC) -c $< $(COMPILER_FLAGS) -MM -MF $@

-include $(DEPS)

$(BUILD_DIR)/%.o: %.cpp
	$(MKDIR_P) $(dir $@)
	$(CC) -c $< $(INCLUDE_PATHS)  $(COMPILER_FLAGS) -o $@ $(LINKER_FLAGS)  $(LIBRARY_PATHS)
